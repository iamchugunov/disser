classdef ToAEKF_RD
    ...
    methods (Access = public)
        function obj = ToAEKF_RD(X0, y, config) % конструктор
           obj.X = X0;
        ...
        function [ S ] = make_S( obj, X, config )
            for i = 1:(size(config.posts,2)-1)
                S(i,1) = sqrt((X(1,1) - config.posts(1,i+1))^2 + (X(3,1) - config.posts(2,i+1))^2 + + (config.hei - config.posts(3,i))^2) - sqrt((X(1,1) - config.posts(1,1))^2 + (X(3,1) - config.posts(2,1))^2 + + (config.hei - config.posts(3,1))^2) ;
            end
        end
        ...
        function [ dS ] = make_dS( obj, X, config )
            for i = 1:(size(config.posts,2)-1)
                d1 = sqrt((X(1,1) - config.posts(1,1))^2 + (X(3,1) - config.posts(2,1))^2 + + (config.hei - config.posts(3,1))^2);
                di = sqrt((X(1,1) - config.posts(1,i+1))^2 + (X(3,1) - config.posts(2,i+1))^2 + + (config.hei - config.posts(3,i+1))^2);
                dS(i,1) = (X(1,1) - config.posts(1,i+1))/di - (X(1,1) - config.posts(1,1))/d1;
                dS(i,2) = 0;
                dS(i,3) = (X(3,1) - config.posts(2,i+1))/di - (X(3,1) - config.posts(2,1))/d1;
                dS(i,4) = 0;
            end
        end
