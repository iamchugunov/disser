classdef ToAEKF_const_perh
    ...
    methods (Access = public)
        function obj = ToAEKF_const_perh(X0, y, config) % конструктор
           obj.X = X0;
           obj.Dx = eye(length(X0));
           obj.sigma_n = config.sigma_n; % м
           obj.sigma_ksi = config.sigma_ksi; % м/с
           obj.D_ksi = eye(3)*config.sigma_ksi^2;
           obj.D_ksi(3,3) = (1e-9*config.c)^2;
           obj.y_prev = y;
        end
...
%             dt = obj.dt_fil;
            ...
            G = [0 0 0; dt 0 0; 0 0 0; 0 dt 0; 0 0 dt];
